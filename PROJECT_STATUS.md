...
**Bug**: `src/Ppg/Ppg.cpp` — todo o pipeline de deteção de batimento cardíaco (`lowPassFilter`/`highPassFilter`/`derivative`/`detectHeartbeat`/`computeBPM`/`smoothBPM`) guardava o seu estado em variáveis `static` **locais a cada função**, nunca repostas por `startHrStreaming()`/`stopHrStreaming()`. Isto não é um cenário raro de "sentar/levantar" — a medição periódica de SpO2 (`SPO2_INTERVAL_MS`, a cada 30s) já **interrompe sempre** o streaming de HR em curso (`stopHrStreaming()` no Passo 2 de `ppgTask`), voltando a chamar `startHrStreaming()` a seguir. Em concreto, `computeBPM()` guardava `lastBeatTime`/`bpm` de uma sessão de streaming anterior: depois de qualquer reinício (garantido sempre que passa uma medição de SpO2 entre dois batimentos), se o primeiro `dt` calculado caísse fora da janela fisiológica 300-2000ms, a função devolvia sem aviso o `bpm` **antigo**, guardado tal como estava — um valor de um contexto fisiológico diferente (ex.: de um episódio de movimento anterior) apresentado ao resto do sistema como uma leitura **nova e válida** (`hr_valid=true`, timestamp atual), incluindo a contaminação subsequente da média móvel em `smoothBPM()` (também nunca reposta). Já existia inclusivamente um `struct HrFilterState` vazio no ficheiro, com o comentário "Mantido apenas para compatibilidade de compilação" — um esboço desta correção que nunca chegou a ser ligado a nada, ficando como dead code até agora. Para um dispositivo cujo propósito é sinalizar frequência cardíaca anómala em doentes com demência, um valor "fantasma" apresentado como leitura fresca é diretamente relevante para a segurança do doente (pode mascarar uma leitura real anómala ou disparar um alerta com base num valor de minutos antes).

**Corrigido**: todo o estado disperso passou a viver num único `struct HrFilterState` (agora com campos reais, não só o comentário-esqueleto) e uma instância `g_hrFilter` de âmbito de ficheiro (dentro do `namespace` anónimo já existente, mesmo padrão de encapsulamento dos restantes globais do módulo). Nova função `resetHrFilterState()` repõe a struct inteira (`g_hrFilter = HrFilterState{};`) e é chamada no início de `startHrStreaming()`, antes de `g_hrStreaming` passar a `true` — cada streaming de HR novo (incluindo o que reinicia a seguir a cada medição de SpO2) arranca sempre com o pipeline de filtros "limpo", tal como acontecia por acidente uma única vez no arranque do firmware (inicialização das antigas `static`), mas agora repetido em cada reinício, não só no boot. O comportamento aritmético de cada filtro não mudou (mesma sequência de leituras/escritas), só passou a ser reiniciável em conjunto.
...
## Dashboard web — revisão dedicada, sem bugs novos confirmados (2026-07-10, rotina dashboard-review)

**Ponto de partida**: `git fetch origin main` + `git pull` (repositório local estava 95 commits atrás — trazido ao dia antes de qualquer análise). Backlog do dashboard (itens 1-10, secção "Backlog de investigação") revisto item a item: **todos já marcados IMPLEMENTADO** por execuções anteriores, incluindo a integração de `medication-reminders.js` no `index.html` (`<script src="medication-reminders.js">`, linha final do ficheiro, feita em 2026-07-07) — ao contrário do que a descrição desta rotina assumia estar por fazer, já não estava. Não há nenhum item do backlog 1-10 nem da secção "Decisões técnicas" pendente que não dependa do bridge, da base de dados ainda por construir, ou de uma decisão do utilizador (provedor de SMS/email real, decisão clínica sobre limiares, etc.).

**Verificação de regressão dirigida** (regra 2 do processo, parser real + browser real):
- `node --check` sobre o `<script>` principal extraído (linhas 849-4537): sem erros de sintaxe.
- Sem IDs HTML duplicados (`grep -oE 'id="..."' | sort | uniq -c`) nem funções/`const`/`let` de topo duplicados.
- Sem handlers `onclick`/`onchange`/etc. mortos (nenhuma chamada em atributo de evento sem função correspondente definida no ficheiro) e sem recorrência do bug já corrigido de aspas escapadas (`\"`) dentro de atributos `onclick`.
- **Teste funcional em browser real** (Playwright/Chromium, `file://.../index.html`, sem bridge a correr — modo demonstração): login como Utente/Família, visita às 8 vistas, sino de notificações, "Medir agora"; logout, login como Médico/Técnico, visita às 8 vistas, troca de paciente (Pacientes → "Selecionar" no 2º paciente) e revisita de Dispositivo/Anomalias/Medicação/Alertas para confirmar que refletem o novo paciente. **Consola sem nenhum erro** além do esperado `WebSocket ... ERR_CONNECTION_REFUSED` (sem bridge a correr, comportamento correto documentado).
- **Countdown do "Medir agora"** (`startForceReadingCountdown()`/`handleCommandResult()`, corrigido em 2026-07-07): testado isoladamente via `page.evaluate()` simulando `liveState.connected=true` + `command_result` — confirmado que `ok:true` a meio do countdown **não** o interrompe (continua a decorrer até aos 15s reais) e que `ok:false` o interrompe de imediato, tal como documentado. **Achado de teste, não bug real**: numa primeira tentativa deste teste, sem desativar o *loop* de reconexão automática do bridge (`scheduleReconnect()`, tenta a cada 4s quando não há bridge), uma tentativa de reconexão falhada em segundo plano chamou `resetPendingCommandButtons()` e interrompeu o countdown simulado a meio — investigado a fundo: só acontece porque o teste forçou `liveState.connected=true`/`sendWsCommand` artificialmente, sem uma ligação `bridgeWs` real por trás. Em uso real isto é impossível de reproduzir: `onForceReadingClick()`/`sendWsCommand()` exigem sempre uma `bridgeWs` genuína e aberta antes de desativar o botão, e o *loop* de reconexão é estritamente sequencial (só agenda a próxima tentativa a partir do `onclose` da anterior) — não há forma de duas ligações coexistirem nem de uma tentativa "órfã" interferir com uma sessão de comando em curso sobre uma ligação diferente e genuinamente ativa. Não reportado como bug (regra: só bugs reais, reproduzíveis, sem esta dependência de um mock parcial) — registado aqui só para não repetir a mesma investigação numa próxima execução.

**Correção aplicada (acessibilidade, pequena e contida)**: os dois botões só-de-ícone da topbar (alternar tema, sino de alertas) tinham `title` mas não `aria-label`, ao contrário do padrão já usado em todo o resto do ficheiro (`role="img"` + `aria-label` em todos os `<canvas>`, `aria-label` em todos os checkboxes de consentimento/permissões). Corrigido: `aria-label` adicionado a ambos (idêntico ao `title` existente); o sino de notificações (`#notifBellBtn`, novo `id`) passou a ter `aria-label` dinâmico, atualizado por `updateNotificationBadge()` — "Alertas" normalmente, "Alertas — há alertas por ler" quando há alertas não lidos do paciente selecionado — em vez de depender só do ponto vermelho visual (`#notifBadgeDot`, agora `aria-hidden="true"` por ser puramente decorativo). **Verificado com Playwright real**: `aria-label` inicial correto para os dois botões; muda para "Alertas — há alertas por ler" com os dados de demonstração da Maria Silva (que têm alertas não lidos); navegação completa nos dois perfis + troca de paciente sem regressões, sem novos erros de consola.
...
**UI do horário/contacto de emergência implementada** (`web/dashboard/index.html`, vista
Definições, entre a equipa de cuidadores e a zona de perigo): cada dia da semana tem a sua
própria janela `{weekday, start, end}` (não um horário único replicado pelos 7 dias — pedido
explícito do utilizador, já que cuidadores com horários regulares podem ter dias de folga
diferentes). Botões de preset "Dias úteis"/"Fim de semana"/"Escolher dias" (mesmo padrão dos
presets de horário recorrente da medicação), com o preset ativo derivado dos dias marcados (nunca
guardado à parte, para nunca dessincronizar de uma edição manual). Suspensão pontual "Hoje estou
disponível" (pedido explícito do utilizador): ignora o horário declarado só para a data de hoje,
sem precisar de editar o horário semanal por causa de uma exceção — expira sozinha no dia
seguinte. Cartão de contacto de emergência (nome/telefone/relação) por paciente. Tudo em
`localStorage`, namespaced por paciente, mesmo padrão de `loadCaregiverTeam()`/`loadConsent()`.
25 chaves i18n novas nos 7 idiomas. Verificado com Playwright: presets marcam os dias certos,
horas ativam/desativam corretamente, suspensão pontual persiste e reflete-se no checkbox após
re-render, contacto de emergência grava, sem fugas de chaves i18n, sem erros de JavaScript;
regressão completa (2 perfis × 7 idiomas × todas as vistas) continua limpa.

**Ainda por fazer** (fora do âmbito desta sessão): ligar `notifications.py`/o horário e contacto
guardados no dashboard a `ble_bridge.py` (`_on_emergency_alert()`) — adiado para depois da Fase 3
(agentes em paralelo estavam a editar `ble_bridge.py`/`storage_advanced.py` ao mesmo tempo, risco
